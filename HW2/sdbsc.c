...
{
    lseek(fd, 0, SEEK_SET); //start at beginning of the file
    student_t temp;
    ...
    return SRCH_NOT_FOUND; //student is not found
}
...
int del_student(int fd, int id)
{
    ...
    // write empty record to the database
    ...
    if (write(fd, &empty_record, STUDENT_RECORD_SIZE) != STUDENT_RECORD_SIZE) {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
...
int count_db_records(int fd)
{
    ...
    while (read(fd, &temp, sizeof(student_t)) > 0) {
        if (temp.id != 0) { // assume id = 0 is empty
            count++;
        }
    }
...
int print_db(int fd)
{
    ...
    while (read(fd, &temp, sizeof(student_t)) > 0) {
        if (temp.id != 0) {
            ...
            printf(STUDENT_PRINT_FMT_STRING, temp.id, temp.fname, temp.lname, temp.gpa / 100.0);
        }
    }
...
{
    // validate student pointer and make sure no empty record
    if (s == NULL || s->id == 0) {
